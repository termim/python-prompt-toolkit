...
Formatted text can be concatenation (or merged) together, with the
:func:`~prompt_toolkit.formatted_text.merge_formatted_text` helper. This helper
can take multiple text objects with either are plain text or formatted text,
and will concatente them into one formatted text object.
